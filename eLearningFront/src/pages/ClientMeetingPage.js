...
jsonWebService
      .get(`${URL_WS}/meeting`)
      .then((response) => {
        let myMeetings = response.data.meetings.filter(m => m.user[0]?._id === userId)
        setMeetingsList(myMeetings)
      })
      .catch((err) => {
      });
  }
  ...
  const checkDate = (meetingDate) => {
    let now = new Date()
    let meetTime = new Date(meetingDate)
    let isReady = now >= meetTime
    if(isReady){
        return true
    }
    else {
  ...
  const testPeerConn = (meetId, roomId) => {
              ...
              <Table>
                ...
                <TableBody>
                  {meetingsList.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { _id, user, category, date, consultant, status, roomId } = row;
                    const isItemSelected = selected.indexOf(_id) !== -1;
                      return (
                        ...
                        >
                          ...
                          <TableCell align="left">{consultant? consultant[0]?.firstName : ''} {consultant? consultant[0]?.lastName : ''}</TableCell>
                          <TableCell align="left">
                            <Iconify
                              icon="ion:call-sharp"
                              width={20}
                              height={20}
                              color={checkDate(date) === true ? checkRoom(roomId) === true ? "#85BB65" : "#ff8000" : "gray"}
                              style={{ cursor: 'pointer' }}
                              onClick= { e => checkDate(date) === true ? checkRoom(roomId) === true ? testPeerConn(_id, roomId) : alert("Please wait untill the room be ready") : alert("Please wait untill the meeting time")}
                            />
                          </TableCell>
                        </TableRow>
                      );
